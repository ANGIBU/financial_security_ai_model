    ...
    def _initialize_data(self):
        ...
        self.korean_subjective_templates = {
            "사이버보안": {
                ...
                "지표_묻기": [
                    "RAT 악성코드의 주요 탐지 지표로는 네트워크 트래픽 모니터링에서 발견되는 비정상적인 외부 통신 패턴, 시스템 동작 분석을 통한 비인가 프로세스 실행 감지, 파일 생성 및 수정 패턴의 이상 징후, 레지스트리 변경 사항 모니터링, 시스템 성능 저하, 의심스러운 네트워크 연결 등이 있습니다. 또한 백그라운드에서 실행되는 미상 서비스, 방화벽 로그의 이상 패턴, 사용자 계정의 비정상적 활동도 중요한 탐지 지표입니다.",
                    "원격 접속 흔적, 의심스러운 네트워크 연결, 시스템 파일 변조, 레지스트리 수정, 비정상적인 메모리 사용 패턴, 알려지지 않은 프로세스 실행, 백그라운드에서 실행되는 미상 서비스 등을 통해 RAT 감염을 탐지할 수 있습니다. 네트워크 연결 로그 분석, 프로세스 모니터링, 파일 무결성 검사, 레지스트리 변경 감시를 통한 종합적 분석이 필요합니다.",
                    "시스템 성능 저하, 예상치 못한 네트워크 활동, 방화벽 로그의 이상 패턴, 파일 시스템 변경 사항, 사용자 계정의 비정상적 활동, 원격 데스크톱 연결 시도, 의심스러운 포트 연결 등이 주요 탐지 지표로 활용됩니다. 실시간 모니터링 시스템을 통해 이러한 지표들을 지속적으로 관찰하고 분석해야 합니다."
                ],
                "복합설명": [
                    "트로이 목마 기반 원격제어 악성코드(RAT)는 정상 프로그램으로 위장하여 사용자가 자발적으로 설치하도록 유도하는 특징을 가지며, 설치 후 외부 공격자가 원격으로 시스템을 제어할 수 있는 백도어를 생성합니다. 은밀성과 지속성을 통해 장기간 시스템에 잠복하면서 악의적인 활동을 수행하며, 시스템 권한 탈취, 데이터 수집, 파일 조작, 원격 명령 실행 등의 기능을 가집니다. 주요 탐지 지표로는 네트워크 트래픽 모니터링에서 발견되는 비정상적인 외부 통신 패턴, 시스템 동작 분석을 통한 비인가 프로세스 실행 감지, 파일 생성 및 수정 패턴의 이상 징후, 레지스트리 변경 사항 모니터링, 시스템 성능 저하, 의심스러운 네트워크 연결 등이 있으며, 이러한 지표들을 종합적으로 분석하여 실시간 탐지와 대응이 필요합니다.",
                    "RAT 악성코드의 특징은 은밀한 설치와 지속적인 원격 제어 능력이며, 트로이 목마 방식으로 배포되어 정상 소프트웨어로 위장하여 탐지를 회피합니다. 시스템 깊숙이 숨어서 장기간 활동하면서 외부 서버와 은밀한 통신을 수행하고 시스템 권한을 탈취합니다. 탐지를 위해서는 네트워크 모니터링을 통한 의심스러운 외부 연결 확인, 프로세스 분석을 통한 비정상 실행 파일 식별, 시스템 파일 무결성 검사, 레지스트리 변경 감시, 실시간 성능 모니터링 등을 수행해야 하며, 이러한 다층적 탐지 체계를 통해 종합적인 보안 관리가 필요합니다."
                ],
                "방안_묻기": [
            ...
            "개인정보보호": {
                "기관_묻기": [
                    "개인정보보호위원회가 개인정보 보호에 관한 업무를 총괄하며, 개인정보침해신고센터에서 신고 접수 및 상담 업무를 담당합니다. 개인정보보호위원회는 개인정보보호법에 따라 설치된 중앙행정기관으로 개인정보 보호 정책 수립과 감시 업무를 수행합니다.",
                    "개인정보보호위원회는 개인정보 보호에 관한 업무를 총괄하는 중앙 행정기관이며, 개인정보 분쟁조정위원회에서 관련 분쟁의 조정 업무를 담당합니다. 개인정보침해신고센터는 개인정보 침해 신고 접수 및 상담을 위한 전문 기관입니다.",
                    "개인정보 침해 관련 신고 및 상담은 개인정보보호위원회 산하 개인정보침해신고센터에서 담당하고 있으며, 피해구제와 분쟁해결을 위한 전문적인 조정 절차를 제공합니다. 개인정보보호위원회는 개인정보보호법에 근거하여 개인정보 보호 정책을 수립하고 감독 업무를 수행합니다."
                ],
                "방안_묻기": [
            ...
            "전자금융": {
                "기관_묻기": [
                    ...
                    "금융감독원 내 전자금융분쟁조정위원회가 전자금융거래법에 따라 이용자의 분쟁조정 신청을 접수하고 처리하는 업무를 수행하며, 전자금융거래에서 발생하는 분쟁의 공정한 해결을 위해 설치된 전문 기구입니다. 전자금융거래와 관련하여 이용자가 피해를 입었을 경우 해당 위원회에 분쟁조정을 신청할 수 있습니다.",
                    "전자금융거래 분쟁의 조정은 금융감독원에 설치된 전자금융분쟁조정위원회에서 담당하며, 전자금융거래법에 근거하여 이용자 보호와 분쟁의 신속한 해결을 위한 업무를 수행합니다. 이용자와 전자금융업자 간에 발생하는 분쟁에 대해 공정하고 전문적인 조정 서비스를 제공합니다."
                ],
                "방안_묻기": [
        ...
        self.institution_database = {
            "전자금융분쟁조정": {
                ...
                "template_answer": "전자금융분쟁조정위원회에서 전자금융거래 관련 분쟁조정 업무를 담당합니다. 이 위원회는 금융감독원 내에 설치되어 운영되며, 전자금융거래법 제28조에 근거하여 이용자와 전자금융업자 간의 분쟁을 공정하고 신속하게 해결하기 위한 업무를 수행합니다."
            },
            "개인정보보호": {
                "기관명": "개인정보보호위원회",
                "소속": "국무총리 소속",
    ...
    def _generate_fallback_templates(self, domain: str, intent_type: str) -> List[str]:
        ...
        domain_fallbacks = {
            "사이버보안": {
                ...
                "복합설명": [
                    "해당 악성코드는 정상 프로그램으로 위장하여 시스템에 침투하고 외부에서 원격으로 제어하는 특성을 가지며, 은밀성과 지속성을 통해 장기간 악의적인 활동을 수행합니다. 주요 탐지 지표로는 비정상적인 네트워크 활동, 시스템 리소스 과다 사용, 알려지지 않은 프로세스 실행, 파일 시스템 변경 등을 실시간 모니터링을 통해 식별할 수 있습니다."
                ]
            },
            "전자금융": {
                "기관_묻기": [
        ...
        general_fallbacks = {
            ...
            "복합설명": [
                "관련 특징과 지표를 종합적으로 분석하여 체계적인 관리 방안을 수립해야 합니다."
            ]
        }
